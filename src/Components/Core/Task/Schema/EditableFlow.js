...
function EditableFlow(props) {
	...
	/**
	 * This function updates the state of the task schema that was previously saved locally.
	 *
	 * @param {Object} schema
	 */
	const onUpdateSchema = () => {
		dispatch(GET_SCHEMA({ userState, taskId: currentTaskState.id, dispatch }));
	};

	/**
	 * TODO: implement the actions needed when a schema template is selected.
	 *
			...
			>
				...
				>
					...
					<DialogContent>
						<DialogContentText>
							<Typography>
								Are you sure you want to change the schema to the{' '}
								<b>{templateSelected}</b> template? This will <b>remove</b> all
								the elements and connections from the current schema in use.
								<br />
								<br />
					...
					<DialogActions>
						...
						<Button
							onClick={() => {
								...
								onUpdateSchema(templates[templateSelected]);
							}}
							color="primary"
						>
							Proceed
