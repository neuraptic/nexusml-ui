		...
		new Promise((resolve) => {
			Papa.parse(file, {
				...
				complete: (results) => {
					...
					if (Object.keys(results.data[0]).length !== allColumns.length - 1) {
					...
					resolve(
						results.data.map((data) => {
							let tmp = {};
							tmp = { ...tmp, status: data.status || 'unlabeled' };
...
export const createCSVTemplate = (allColumns) => {
	...
	allColumns.forEach((column) => {
		if (column.field !== 'edit' && column.field !== 'comments') {
			tmpHeaders.push({
				...tmpHeaders[0],
				key: column.field,
				label: column.field,
...
export const parseCSVTimeSeriesFile = async (
	file,
	allColumns,
	dispatch,
	setIsLoadingFile,
				...
				complete: (results) => {
					...
					if (
						...
						dispatch(
							ADD_ALERT({
								type: 'warning',
								message: 'The provided data includes no numeric values.',
							})
						);
						setTimeout(() => {
							dispatch(
								REMOVE_ALERT('The provided data includes no numeric values.')
							);
						}, 5000);
						setFileToUpload(null);
						setIsLoadingFile(false);
					...
					resolve(results.data[0].filter((data) => data).map((data) => data));
				},
			});
		});
