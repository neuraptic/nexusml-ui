...
import { createValidatedTestingObjects } from '../../aipredictions.services';
...
export const CreateOrUpdateAIPredictionModal = (props) => {
	...
	const handleSaveCreate = async () => {
		...
		if (creationStep === 1) {
			...
			const tmpPrediction = manualExample.values
				.filter((val) => {
				...
				if (tmp) {
					...
					if (column.fieldType === 'input') {
						if (column.valueType === 'category') {
							categoriesState.inputs.forEach((category) => {
								if (category.categories.find((cat) => cat.id === tmp.value))
									tmpCat = category.categories.find(
										(cat) => cat.id === tmp.value
									).name;
							});
							formatedPrediction.inputs.push({
								...tmp,
								value: tmpCat,
							});
						} else formatedPrediction.inputs.push(tmp);
					}
					if (column.fieldType === 'metadata') {
						if (column.valueType === 'category') {
							categoriesState.metadata.forEach((category) => {
								if (category.categories.find((cat) => cat.id === tmp.value))
									tmpCat = category.categories.find(
										(cat) => cat.id === tmp.value
									).name;
							});
							formatedPrediction.metadata.push({
								...tmp,
								value: tmpCat,
							});
						} else formatedPrediction.metadata.push(tmp);
					}
	...
	const handleChangeUploadCSV = async (event) => {
		...
		const newPredictions = createValidatedTestingObjects(
			await parseCSVFile({
		...
		if (!res.errors) {
			const formatedPrediction = res.map((prediction) => {
				const tmpPrediction = {
					id: uuidv4(),
					inputs: [],
					metadata: [],
				...
				allColumns.forEach((column) => {
					const tmpElement = prediction.values.find(
						(el) =>
							el.element === column.name || el.element === column.display_name
					);
					...
					if (tmpElement) {
						...
						if (column.fieldType === 'input') {
							if (column.valueType === 'category') {
								categoriesState.inputs.forEach((category) => {
									if (category.name === tmpElement.element)
										tmpCat = category.categories.find(
											(cat) => cat.name === tmpElement.value
										).name;
								});
								tmpPrediction.inputs.push({
									...tmpElement,
									value: tmpCat,
								});
							} else tmpPrediction.inputs.push(tmpElement);
						}
						if (column.fieldType === 'metadata') {
							if (column.valueType === 'category') {
								categoriesState.metadata.forEach((category) => {
								...
								tmpPrediction.metadata.push({
									...tmpElement,
									value: tmpCat,
								});
							} else tmpPrediction.metadata.push(tmpElement);
						}
					}
				});
													...
													}
													{
														...
														column.valueType === 'float' &&
															(!column.multiValue ? (
																...
																<div className={cssStyles.uploadContainer}>
																	{manualExample.values.find(
																		(element) =>
																			element.element === column.field
																	)?.isLoading ? (
...
CreateOrUpdateAIPredictionModal.propTypes = {
	open: PropTypes.bool,
