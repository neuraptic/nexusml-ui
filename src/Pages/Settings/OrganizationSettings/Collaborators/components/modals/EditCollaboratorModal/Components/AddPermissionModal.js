...
export const AddPermissionModal = ({
	open,
	setOpen,
	handleAddCollaboratorPermission,
}) => {
	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			...
			if (!hasError) {
				...
				handleAddCollaboratorPermission(values);
					...
					<div>
						<Box sx={styles().dialogContentContainer}>
							...
							<Box sx={{ ...styles().formInput, maxWidth: '70%' }}>
								<Box sx={{ marginLeft: 1, width: 300, display: 'flex' }}>
									<FormControl>
										<div>Allow</div>
										<div>
											<Checkbox
...
AddPermissionModal.propTypes = {
	...
	handleAddCollaboratorPermission: PropTypes.func,
};
